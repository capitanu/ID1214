...
class Snake(object):
    ...
    def state_info_2(self, snack):
        ...
        # right
        #calin    distance = (self.rows - self.head.pos[0]) / self.rows
        distance_toWall = (self.rows - self.head.pos[0]) / self.rows
        distance_toSelf = 0
        distance_toSnack = 0
        snack_available_1 = 0
        if snack.pos[1] == self.head.pos[1] and snack.pos[0]  > self.head.pos[0]:
            snack_available_1 = 1
            distance_toSnack = snack.pos[0] - self.head.pos[0] / self.rows
        body_in_range = 0
        for i in range(self.head.pos[0] + 1, self.rows):
            if self.block_in_body_except_head((i, self.head.pos[1])):
                ...
                if(i < snack.pos[0]):
                    snack_available_1 = 0
                    distance_toSelf = i - self.head.pos[0] / self.rows
                    body_in_range = 1
                break
        if snack_available_1 == 1:
            distance = distance_toSnack
        elif snack_available_1 == 0 and body_in_range == 1 :
        ...
        distance_toWall = (self.head.pos[1]) / self.rows
        distance_toSelf = 0
        distance_toSnack = 0
        snack_available_2 = 0
        if snack.pos[0] == self.head.pos[0] and snack.pos[1] < self.head.pos[1]:
            snack_available_2 = 1
            distance_toSnack = self.head.pos[1] - snack.pos[1]
        body_in_range = 0
        for i in range(0, self.head.pos[1]):
            if self.block_in_body_except_head((self.head.pos[0], i)):
                if(i > snack.pos[1]):
                    snack_available_2 = 0
                    distance_toSelf = self.head.pos[1] - i
                body_in_range = 1
                break
        if snack_available_2 == 1:
            distance = distance_toSnack
        elif snack_available_2 == 0 and body_in_range == 1 :
            distance = distance_toSelf
        else: distance = distance_toWall
        ...
        # left
        distance_toWall = (self.head.pos[0]) / self.rows
        distance_toSelf = 0
        distance_toSnack = 0
        snack_available_3 = 0
        if snack.pos[1] == self.head.pos[1] and snack.pos[0] < self.head.pos[0]:
            snack_available_3 = 1
            distance_toSnack = self.head.pos[0] - snack.pos[0]
        body_in_range = 0
        for i in range(0,self.head.pos[0]):
            if self.block_in_body_except_head((i, self.head.pos[1])):
                if(i > snack.pos[0]):
                    snack_available_3 = 0
                    distance_toSelf = self.head.pos[0] - i
                body_in_range = 1
                break
        if snack_available_3 == 1:
            distance = distance_toSnack
        elif snack_available_3 == 0 and body_in_range == 1 :
            distance = distance_toSelf
        else: distance = distance_toWall
        ...
        if snack.pos[0] == self.head.pos[0] and snack.pos[1]  > self.head.pos[1]:
            snack_available_4 = 1
            distance_toSnack = snack.pos[1] - self.head.pos[1]
        body_in_range = 0
        for i in range(self.head.pos[1] + 1,self.rows):
            if self.block_in_body_except_head((self.head.pos[0], i)):
                if(i < snack.pos[1]):
                    snack_available_4 = 0
                    distance_toSnack = i - self.head.pos[1]
                body_in_range = 1
                break
        if snack_available_4 == 1:
            distance = distance_toSnack
        elif snack_available_4 == 0 and body_in_range == 1 :
            distance = distance_toSelf
        else: distance = distance_toWall
        ...
        while(i < self.rows and j > 0):
            ...
            if self.block_in_body_except_head((i,j)):
                body_in_range = 1
                distance_toSelf = math.sqrt(math.pow(i - self.head.pos[0] , 2) + math.pow(self.head.pos[1] - j, 2))
                break
            if snack.pos[0] == i and snack.pos[1] == j:
                snack_available = 1
                distance_toSnack = math.sqrt(math.pow(i - self.head.pos[0] , 2) + math.pow(self.head.pos[1] - j, 2))
                break
        distance_toWall = math.sqrt(math.pow(self.head.pos[0] - self.rows + 1 , 2 ) + math.pow(self.head.pos[1], 2))
        if snack_available == 1:
            distance = distance_toSnack
        elif body_in_range == 1 :
            distance = distance_toSelf
        elif snack_available == 1 and body_in_range == 1 :
        ...
        while(i > 0 and j > 0):
            ...
            if self.block_in_body_except_head((i,j)):
                body_in_range = 1
                distance_toSelf = math.sqrt(math.pow(self.head.pos[0] - i, 2) + math.pow(self.head.pos[1] - j, 2))
                break
            if snack.pos[0] == i and snack.pos[1] == j:
                snack_available = 1
                distance_toSnack = math.sqrt(math.pow(self.head.pos[0] - i, 2) + math.pow(self.head.pos[1] - j, 2))
                break
        distance_toWall = math.sqrt(math.pow(self.head.pos[0] , 2 ) + math.pow(self.head.pos[1], 2))
        if snack_available == 1:
            distance = distance_toSnack
        elif body_in_range == 1 :
            distance = distance_toSelf
        ...
        while(i > 0 and j < self.rows):
            ...
            if self.block_in_body_except_head((i,j)):
                body_in_range = 1
                distance_toSelf = math.sqrt(math.pow(self.head.pos[0] - i, 2) + math.pow(j - self.head.pos[1], 2))
                break
            if snack.pos[0] == i and snack.pos[1] == j:
                snack_available = 1
                distance_toSnack = math.sqrt(math.pow(self.head.pos[0] - i, 2) + math.pow(j - self.head.pos[1], 2))
                break
        distance_toWall = math.sqrt(math.pow(self.head.pos[0] , 2 ) + math.pow(self.head.pos[1] - self.rows + 1, 2))
        if snack_available == 1:
            distance = distance_toSnack
        elif body_in_range == 1 :
            distance = distance_toSelf
        ...
        while(i < self.rows and j < self.rows):
            ...
            if self.block_in_body_except_head((i,j)):
                body_in_range = 1
                distance_toSelf = math.sqrt(math.pow(i - self.head.pos[0], 2) + math.pow(j - self.head.pos[1], 2))
                break
            if snack.pos[0] == i and snack.pos[1] == j:
                snack_available = 1
                distance_toSnack = math.sqrt(math.pow(i - self.head.pos[0], 2) + math.pow(j - self.head.pos[1], 2))
                break
        distance_toWall = math.sqrt(math.pow(self.head.pos[0] - self.rows + 1 , 2 ) + math.pow(self.head.pos[1] - self.rows + 1, 2))
        if snack_available == 1:
            distance = distance_toSnack
        elif body_in_range == 1 :
            distance = distance_toSelf
